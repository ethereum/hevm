  ...
  , testGroup "simplify-storage"
    [ test "simplify-storage-array-only-static" $ do
       Just c <- solcRuntime "MyContract"
        ...
        |]
       expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "transfer(uint256,uint256,uint256)" [AbiUIntType 256, AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
    ...
    , expectFail $ test "simplify-storage-array-symbolic-index" $ do
       Just c <- solcRuntime "MyContract"
        ...
        |]
       expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "transfer(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
    ...
    , test "simplify-storage-array-loop-nonstruct" $ do
       Just c <- solcRuntime "MyContract"
        ...
        |]
       expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "transfer(uint256)" [AbiUIntType 256])) [] (defaultVeriOpts { maxIter = Just 5 })
       assertEqualM "Expression is not clean." (badStoresInExpr expr) False
    , test "simplify-storage-map-newtest1" $ do
       Just c <- solcRuntime "MyContract"
        ...
        |]
       expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
       assertEqualM "Expression is not clean." (badStoresInExpr expr) False
       (_, [(Qed _)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x11] c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
       liftIO $ putStrLn "OK"
    , ignoreTest $ test "simplify-storage-map-todo" $ do
       ...
       (_, [Cex _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
       liftIO $ putStrLn "OK"
    , test "simplify-storage-array-loop-struct" $ do
       Just c <- solcRuntime "MyContract"
        ...
        |]
       expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "transfer(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] (defaultVeriOpts { maxIter = Just 5 })
       assertEqualM "Expression is not clean." (badStoresInExpr expr) False
    , test "decompose-1" $ do
      Just c <- solcRuntime "MyContract"
        [i|
        ...
        |]
      expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "prove_mapping_access(address,address)" [AbiAddressType, AbiAddressType])) [] defaultVeriOpts
      putStrLnM $ T.unpack $ formatExpr expr
    ...
    , test "decompose-2" $ do
      Just c <- solcRuntime "MyContract"
        ...
        |]
      expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "prove_mixed_symoblic_concrete_writes(address,uint256)" [AbiAddressType, AbiUIntType 256])) [] defaultVeriOpts
      let simpExpr = mapExprM Expr.decomposeStorage expr
      -- putStrLnM $ T.unpack $ formatExpr (fromJust simpExpr)
      assertEqualM "Decompose did not succeed." (isJust simpExpr) True
    , test "decompose-3" $ do
      Just c <- solcRuntime "MyContract"
        ...
        |]
      expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "prove_array(uint256,uint256,uint256,uint256)" [AbiUIntType 256, AbiUIntType 256, AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
      let simpExpr = mapExprM Expr.decomposeStorage expr
      assertEqualM "Decompose did not succeed." (isJust simpExpr) True
    ...
    , test "decompose-5-mixed" $ do
      Just c <- solcRuntime "MyContract"
        ...
        |]
      expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "prove_mixed(address,address,uint256)" [AbiAddressType, AbiAddressType, AbiUIntType 256])) [] defaultVeriOpts
      let simpExpr = mapExprM Expr.decomposeStorage expr
      -- putStrLnM $ T.unpack $ formatExpr (fromJust simpExpr)
      assertEqualM "Decompose did not succeed." (isJust simpExpr) True
    ...
    , expectFail $ test "decompose-6-fail" $ do
      Just c <- solcRuntime "MyContract"
        ...
        |]
      expr <- withSolvers CVC5 1 Nothing $ \s -> getExpr s c (Just (Sig "prove_mixed(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
      let simpExpr = mapExprM Expr.decomposeStorage expr
      -- putStrLnM $ T.unpack $ formatExpr (fromJust simpExpr)
      assertEqualM "Decompose did not succeed." (isJust simpExpr) True
 ...
 , testGroup "Panic code tests via symbolic execution"
  [
     test "assert-fail" $ do
       Just c <- solcRuntime "MyContract"
           ...
           |]
       (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x01] c (Just (Sig "fun(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
     ...
     test "safeAdd-fail" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x11] c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
     ...
     test "div-by-zero-fail" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x12] c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
     ...
     ,
      test "unused-args-fail" $ do
         Just c <- solcRuntime "C"
             ...
             |]
         (_, [Cex _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x1] c Nothing [] defaultVeriOpts
         putStrLnM "expected counterexample found"
      ,
     test "enum-conversion-fail" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x21] c (Just (Sig "fun(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
     ...
     ignoreTest $ test "pop-empty-array" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        a <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x31] c (Just (Sig "fun(uint8)" [AbiUIntType 8])) [] defaultVeriOpts
        liftIO $ do
          print $ length a
          print $ show a
          putStrLnM "expected counterexample found"
     ...
     test "access-out-of-bounds-array" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Cex (_, _)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x32] c (Just (Sig "fun(uint8)" [AbiUIntType 8])) [] defaultVeriOpts
      ...
      test "alloc-too-much" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Cex _]) <- withSolvers CVC5 1 Nothing $ \s ->
      ...
      expectFail $ test "call-zero-inited-var-thats-a-function" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Cex (_, cex)]) <- withSolvers CVC5 1 Nothing $
  ...
  , testGroup "Symbolic-Constructor-Args"
    ...
    [ test "multiple-symbolic-constructor-calls" $ do
        Just initCode <- solidity "C"
          ...
          |]
        withSolvers CVC5 1 Nothing $ \s -> do
  ...
  , testGroup "max-iterations"
    [ test "concrete-loops-reached" $ do
        ...
        (e, [Qed _]) <- withSolvers CVC5 1 Nothing $
  ...
  , testGroup "Symbolic Addresses"
    [ test "symbolic-address-create" $ do
        ...
        (expr, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s ->
  ...
  , testGroup "Symbolic execution"
     ...
     test "require-test" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(int256)" [AbiIntType 256])) [] defaultVeriOpts
     ...
     test "ITE-with-bitwise-AND" $ do
       ...
       (_, [Cex _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "f(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
       putStrLnM "expected counterexample found"
     ,
     test "ITE-with-bitwise-OR" $ do
       Just c <- solcRuntime "C"
         ...
         |]
       (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "f(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
    ...
    test "copyslice-check" $ do
      ...
      (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s ->
     ...
     test "opcode-mul-assoc" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(int256,int256,int256)" [AbiIntType 256, AbiIntType 256, AbiIntType 256])) [] defaultVeriOpts
     ...
     test "opcode-sar-neg" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(int256,int256)" [AbiIntType 256, AbiIntType 256])) [] defaultVeriOpts
     ...
     test "signed-overflow-checks" $ do
        Just c <- solcRuntime "C"
            ...
            |]
        (_, [Cex (_, _)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x11] c (Just (Sig "fun(int256)" [AbiIntType 256])) [] defaultVeriOpts
     ...
     test "opcode-signextend-neg" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _])  <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "foo(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
     ...
     test "opcode-signextend-pos-nochop" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256,uint8)" [AbiUIntType 256, AbiUIntType 8])) [] defaultVeriOpts
        putStrLnM "signextend works as expected"
      ,
     ...
     test "opcode-shl" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
        putStrLnM "SAR works as expected"
     ,
     test "opcode-xor-cancel" $ do
      ...
      test "opcode-add-commutative" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        a <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
        case a of
          (_, [Cex (_, ctr)]) -> do
            let x = getVar ctr "arg1"
            let y = getVar ctr "arg2"
      ...
      test "opcode-div-res-zero-on-div-by-zero" $ do
        Just c <- solcRuntime "MyContract"
            ...
            |]
        (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint16)" [AbiUIntType 16])) [] defaultVeriOpts
     ...
     ,
      ...
      test "summary storage writes" $ do
        ...
        test "Neg" $ do
            ...
            (res, [Qed _]) <- withSolvers CVC5 4 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "hello(address)" [AbiAddressType])) [] defaultVeriOpts
            putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        test "catch-storage-collisions-noproblem" $ do
          ...
          (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s ->
            verifyContract s c sig [] defaultVeriOpts (Just pre) (Just post)
          ...
          let pre vm = (Lit 0) .== vm.state.callvalue
              post prestate leaf =
                ...
                in case leaf of
                     ...
                     _ -> PBool True
              sig = Just (Sig "f(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])
          (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s ->
            verifyContract s c sig [] defaultVeriOpts (Just pre) (Just post)
          let x = getVar ctr "arg1"
          let y = getVar ctr "arg2"
          putStrLnM $ "y:" <> show y
        ...
        test "simple-assert" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (_, [Cex (Failure _ _ (Revert msg), _)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "foo()" [])) [] defaultVeriOpts
        ...
        test "simple-assert-2" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (_, [(Cex (_, ctr))]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "foo(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
        ...
        test "assert-fail-equal" $ do
          Just c <- solcRuntime "AssertFailEqual"
            ...
            |]
          (_, [Cex (_, a), Cex (_, b)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
        ...
        test "assert-fail-twoargs" $ do
          Just c <- solcRuntime "AssertFailTwoParams"
            ...
            |]
          (_, [Cex _, Cex _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
          putStrLnM "expected 2 counterexamples found"
        ,
        test "assert-2nd-arg" $ do
          Just c <- solcRuntime "AssertFailTwoParams"
            ...
            |]
          (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "fun(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
        ...
        test "check-lsb-msb1" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "foo(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
          putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        ...
        test "Deposit contract loop (z3)" $ do
          Just c <- solcRuntime "Deposit"
            ...
            |]
          (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "deposit(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
          putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        test "Deposit-contract-loop-error-version" $ do
          Just c <- solcRuntime "Deposit"
            ...
            |]
          (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s allPanicCodes c (Just (Sig "deposit(uint8)" [AbiUIntType 8])) [] defaultVeriOpts
        ...
        test "explore function dispatch" $ do
          Just c <- solcRuntime "A"
            ...
            |]
          (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c Nothing [] defaultVeriOpts
          putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        test "check-asm-byte-in-bounds" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (_, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c Nothing [] defaultVeriOpts
        ...
        test "injectivity of keccak (32 bytes)" $ do
          Just c <- solcRuntime "A"
            ...
            |]
          (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "f(uint256,uint256)" [AbiUIntType 256, AbiUIntType 256])) [] defaultVeriOpts
          putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        ...
        test "injectivity of keccak (64 bytes)" $ do
          Just c <- solcRuntime "A"
            ...
            |]
          (_, [Cex (_, ctr)]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "f(uint256,uint256,uint256,uint256)" (replicate 4 (AbiUIntType 256)))) [] defaultVeriOpts
          let x = getVar ctr "arg1"
          let y = getVar ctr "arg2"
        ...
        test "calldata beyond calldatasize is 0 (concrete dalldata prefix)" $ do
          Just c <- solcRuntime "A"
            ...
            |]
          (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "f(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
          putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        ...
        test "multiple-contracts" $ do
          ...
          (_, [Cex (_, cex)]) <- withSolvers CVC5 1 Nothing $ \s -> do
        ...
        expectFail $ test "calling unique contracts (read from storage)" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (_, [Cex _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "call_A()" [])) [] defaultVeriOpts
          putStrLnM "expected counterexample found"
        ,
        test "keccak-concrete-and-sym-agree" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (res, [Qed _]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s defaultPanicCodes c (Just (Sig "kecc(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
          putStrLnM $ "successfully explored: " <> show (Expr.numBranches res) <> " paths"
        ,
        ...
        test "safemath-distributivity-yul" $ do
          ...
          (_, [Qed _]) <-  withSolvers CVC5 1 Nothing $ \s -> verify s defaultVeriOpts vm (Just $ checkAssertions defaultPanicCodes)
          putStrLnM "Proven"
        ,
        ...
        test "storage-cex-1" $ do
          Just c <- solcRuntime "C"
            ...
            |]
          (_, [(Cex (_, cex))]) <- withSolvers CVC5 1 Nothing $ \s -> checkAssert s [0x01] c (Just (Sig "fun(uint256)" [AbiUIntType 256])) [] defaultVeriOpts
    ...
    [
      test "eq-yul-simple-cex" $ do
        ...
        Just bPrgm <- liftIO $ yul ""
          ...
          |]
        withSolvers CVC5 3 Nothing $ \s -> do
...
checkEquivBase mkprop l r = do
  conf <-  readConfig
  withSolvers CVC5 1 (Just 1) $ \solvers -> liftIO $ do
    if l == r
       then do
         putStrLnM "skip"
         pure True
...
checkPost post c sig = do
  (e, res) <- withSolvers CVC5 1 Nothing $ \s ->
