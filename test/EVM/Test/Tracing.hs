...
runCodeWithTrace rpcinfo evmEnv alloc txn fromAddr toAddress = withSolvers Z3 0 Nothing $ \solvers -> do
  ...
  origVM <- liftIO $ stToIO $ vmForRuntimeCode (Proxy @Word64) code' calldata' evmEnv alloc txn fromAddr toAddress
...
execWithTrace :: App m => StateT (TraceState RealWorld) m (VMResult Word64 RealWorld)
execWithTrace = do
  _ <- runWithTrace
  fromJust <$> use (_1 % #result)

runWithTrace :: App m => StateT (TraceState RealWorld) m (VM Word64 RealWorld)
runWithTrace = do
...
interpretWithTrace
  :: forall m a . App m
  => Fetch.Fetcher m Word64 RealWorld
