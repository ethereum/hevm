...
import EVM (StorageBase (..), balance, contractcode, initialContract, nonce, origStorage, storage)
...
import EVM.Solvers (Solver (Z3), withSolvers)
...
checkStateFail diff x vm (okMoney, okNonce, okData, okCode) = do
  let printContracts :: Map Addr (EVM.Contract, Storage) -> IO ()
      printContracts cs =
        putStrLn $
          Map.foldrWithKey
            ( \k (c, s) acc ->
...
checkExpectation diff x vm = do
  ...
  if okState
    then pure Nothing
    else Just <$> checkStateFail diff x vm (b2, b3, b4, b5)
...
parseBCSuite x = case (JSON.eitherDecode' x) :: Either String (Map String BlockchainCase) of
  Left e -> Left e
  Right bcCases ->
    let allCases = fromBlockchainCase <$> bcCases
...
validateTx tx block cs = do
  ...
  if gasDeposit + tx.txValue <= originBalance
    && tx.txNonce == originNonce
    && block.blockBaseFee <= maxBaseFee tx
    then Just ()
    else Nothing
...
vmForCase x =
  let a = x.checkContracts
      cs = Map.map fst a
      st = Map.mapKeys num $ Map.map snd a
      vm =
        EVM.makeVm x.testVmOpts
