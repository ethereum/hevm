  ...
  testGroup
    ...
    [ testGroup
        ...
        [ testCase "pre-merge-block" $ do
            ...
            (cb, numb, basefee, prevRan) <-
              fetchBlockFrom block' testRpc >>= \case
      ...
      testGroup
        ...
        [ testCase "dapp-test" $ do
          ...
          testCase "weth-conc" $ do
            ...
            let postStore = case view (env . storage) postVm of
                  ConcreteStore s -> s
...
weth9VM blockNum calldata' = do
  let caller' = Lit 0xf04a5cc80b1e94c69b48f5ee68a08cd2f09a7c3e
...
vmFromRpc blockNum calldata' callvalue' caller' address' = do
  ctrct <-
    fetchContractFrom (BlockNumber blockNum) testRpc address' >>= \case
      Nothing -> error $ "contract not found: " <> show address'
      Just contract' -> return contract'

  blk <-
    fetchBlockFrom (BlockNumber blockNum) testRpc >>= \case
      Nothing -> error "could not fetch block"
      Just b -> pure b
