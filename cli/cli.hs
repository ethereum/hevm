...
import Options.Applicative.Help as Help
-- import Data.Semoid (Monoid)
...
data Command2
  = Test2 CommonOptions TestOptions
  | Eq2 CommonOptions EqOptions
  | Exec2 CommonOptions ExecOptions

-- Parser for common options
commonOptions :: Parser CommonOptions
commonOptions = CommonOptions
  <$> option auto ( long "verbosity" <> short 'v'
    <> help "Set verbosity level" <> showDefault
    <> value 1 <> Options.Applicative.metavar "INT" )

-- Parser for the 'test' subcommand
testOptions :: Parser TestOptions
testOptions = TestOptions
  <$> strOption ( long "code" <> short 'c' <> help "Code to test" )

-- Parser for the 'eq' subcommand
eqOptions :: Parser EqOptions
eqOptions = EqOptions
  <$> strOption ( long "code1" <> short 'a' <> help "First code to compare" <> Options.Applicative.metavar "STRING" )
  <*> strOption ( long "code2" <> short 'b' <> help "Second code to compare" <> Options.Applicative.metavar "STRING" )

-- Parser for the 'exec' subcommand
execOptions :: Parser ExecOptions
execOptions = ExecOptions
  <$> strOption ( long "code" <> short 'c'
    <> help "Code to execute" <> Options.Applicative.metavar "STRING" )

-- Parser for the subcommands
commandParser :: Parser Command2
commandParser = subparser
  ( command "test"
      (info (Test2 <$> commonOptions <*> testOptions <**> helper)
        (progDesc "Run tests"))
  <> command "eq"
      (info (Eq2 <$> commonOptions <*> eqOptions<**> helper)
        (progDesc "Compare two code snippets"
        <> header "example - a command-line tool with subcommands"
        <> footer "Bimm example - a command-line tool with subcommands" )
      )
  <> command "exec"
      (info (Exec2 <$> commonOptions <*> execOptions<**> helper)
        (progDesc "Execute a code snippet"))
  )
...
main = do
  cmd <- execParser $ info (commandParser <**> helper)
    ...
    <> progDesc "A program with multiple subcommands"
    <> footer "XYZexample - a command-line tool with subcommands"
    <> headerDoc (Just ("XYZexample - a command-line tool with subcommands"))
    )
  case cmd of
    Test2 commonOpts testOpts -> do
      putStrLn $ "Running test with verbosity " ++ show commonOpts.verbosity
      putStrLn $ "Code: " ++ testOpts.testCode
    Eq2 commonOpts eqOpts -> do
      putStrLn $ "Comparing code with verbosity " ++ show commonOpts.verbosity
      putStrLn $ "Code 1: " ++ eqOpts.eqCode1
      putStrLn $ "Code 2: " ++ eqOpts.eqCode2
    Exec2 commonOpts execOpts -> do
      putStrLn $ "Executing code with verbosity " ++ show commonOpts.verbosity
      putStrLn $ "Code: " ++ execOpts.execCode
