...
import Control.Lens hiding (passing, pre)
...
import EVM.Dapp (DappInfo, dappInfo, emptyDapp, findUnitTests)
import EVM.Debug
...
import EVM.Format (formatExpr, showTraceTree)
import EVM.SMT
import EVM.Solidity
import EVM.Solvers
...
import EVM.UnitTest (UnitTestOptions, coverageForUnitTestContract, coverageReport, dappTest, getParametersFromEnvironmentVariables, testNumber)
import EVM.UnitTest qualified
import GHC.Conc
import GHC.Natural
    ...
    >>= \case
      Just (contractMap, sourceCache) -> do
        ...
        covs <-
          mconcat
            <$> mapM
              (coverageForUnitTestContract opts contractMap sourceCache)
              unitTests
        let dapp = dappInfo "." contractMap sourceCache
            f (k, vs) = do
...
vmFromCommand cmd = do
  ...
  (miner, ts, baseFee, blockNum, prevRan) <- case cmd.rpc of
    ...
    Just url ->
      EVM.Fetch.fetchBlockFrom block' url >>= \case
  ...
  where
    ...
    vm0 baseFee miner ts blockNum prevRan c =
      EVM.makeVm $
        EVM.VMOpts
          { vmoptContract = c,
...
symvmFromCommand cmd calldata' = do
        ...
        Just contract' -> return contract''
          where
            contract'' = case cmd.code of
              ...
              -- fetch the contract and overwrite the code
              Just c ->
                EVM.initialContract (mkCode $ decipher c)
                  -- TODO: fix this
  ...
  return $
    (EVM.Transaction.initTx $ withCache $ vm0 baseFee miner ts blockNum prevRan calldata' callvalue' caller' contract')
  ...
  where
    ...
    vm0 baseFee miner ts blockNum prevRan cd' callvalue' caller' c =
      EVM.makeVm $
        EVM.VMOpts
          { vmoptContract = c,
            vmoptCalldata = cd',
