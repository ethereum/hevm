...
import Control.Lens hiding (Indexed, List, elements, passing)
...
import Data.Text (Text, intercalate, isPrefixOf, pack, stripSuffix, unpack)
...
import EVM.SymExec (VeriOpts, defaultVeriOpts, extractCex, isQed, runExpr, subModel, symCalldata, verify)
    ...
    Just (contractMap, _) -> do
      ...
      case cache' of
        ...
        Just path ->
          ...
          let evmcache = mconcat [vm._cache | vm <- vms]
           in liftIO $ Git.saveFacts (Git.RepoAt path) (Facts.cacheFacts evmcache)
...
coverageReport dapp cov =
  let sources :: SourceCache
   ...
   in Map.mapWithKey f linesByName
  ...
  (name, testNames) = do
    ...
    case Map.lookup name contractMap of
      ...
      Just theContract -> do
        ...
        let runOne' (test, _) = spawn_ . liftIO $ do
              (_, (_, cov)) <-
                runStateT
    ...
    case Map.lookup name contractMap of
      ...
      Just theContract -> do
        ...
        vm1 <-
          liftIO $
            execStateT
              ( EVM.Stepper.interpret
                  (Fetch.oracle solvers rpcInfo)
                  (Stepper.enter name >> initializeUnitTest opts theContract)
              )
              vm0
              ...
              selected =
                [ ( addr,
                    fromMaybe (error ("no src found for: " <> show addr)) $
                      lookupCode
                        ( fromMaybe (error $ "contract not found: " <> show addr) $
                            Map.lookup addr cs
                        )._contractcode
                        dapp
                  )
...
exploreRun opts@UnitTestOptions {..} initialVm testName replayTxs = do
  ...
  if x
    then
      return
        ( "\x1b[32m[PASS]\x1b[0m " <> testName <> " (runs: " <> (pack $ show fuzzRuns) <> ", depth: " <> pack (show depth) <> ")",
...
runOne opts@UnitTestOptions {..} vm testName args = do
  ...
  (success, vm'') <-
    runStateT
      (EVM.Stepper.interpret (Fetch.oracle solvers rpcInfo) (checkFailures opts testName bailed))
      vm'
  if success
    then
...
symRun opts@UnitTestOptions {..} vm testName types = do
  ...
  let failed store =
        (And (readStorage' (litAddr testContract) (Lit 0) store) (Lit 2) .== Lit 2)
...
symFailure UnitTestOptions {..} testName cd types failures' =
  ...
  where
    ...
    showRes = \case
      Return _ _ _ ->
        if "proveFail" `isPrefixOf` testName
...
getParametersFromEnvironmentVariables rpc = do
  ...
  (miner, ts, blockNum, ran, limit, base) <-
    case rpc of
      ...
      Just url ->
        Fetch.fetchBlockFrom block' url >>= \case
  ...
  let getWord s def = maybe def read <$> lookupEnv s
