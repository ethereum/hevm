    ...
    rlpAccessList =
      EVM.RLP.List $
        map
          ( \accessEntry ->
              EVM.RLP.List
                [ BS $ word160Bytes accessEntry.accessAddress,
...
setupTx origin coinbase gasPrice gasLimit prestate =
  let gasCost = gasPrice * (num gasLimit)
   in ( Map.adjust
          ( (over EVM.nonce (+ 1))
              . (over balance (subtract gasCost))
          )
          origin
      )
        . touchAccount origin
        . touchAccount coinbase
        $ prestate
...
initTx vm =
  let toAddr = vm._state._contract
      ...
      initState =
        ( case unlit value of
            Just v ->
              ((Map.adjust (over balance (subtract v))) origin)
