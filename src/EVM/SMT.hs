...
assertProps conf ps = assertPropsNoSimp conf withSepStore
  where
    simplifiedProps = trace "YO" $ Expr.simplifyProps ps
    withSepStore = trace "OI" $ case mapM (mapPropM Expr.decomposeStorage) simplifiedProps of
                     Just ps' -> trace "REWROTE" ps'
                     Nothing -> trace "NO REWRITE" simplifiedProps
