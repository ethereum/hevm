...
import Data.Binary.Get (Get, getWord32be, getWord8, label, runGet, runGetOrFail, skip)
...
import Data.DoubleWord (Int256, Word256, signedWord)
import Data.Functor (($>))
...
import EVM.Expr (isLitWord, readWord)
import EVM.Types
import GHC.Generics
    ...
    putHeads offset (x : xs') =
      case abiKind (abiValueType x) of
        Static -> do
          ...
          putHeads offset xs'
        Dynamic -> do
          putAbi (AbiUInt 256 (fromIntegral offset))
...
typeWithArraySuffix v = do
  ...
  let parseSize :: AbiType -> String -> AbiType
...
asUInt n f = y <$> getAbi (AbiUIntType n)
  where
    y (AbiUInt _ x) = f (fromIntegral x)
...
genAbiValue = \case
  ...
  AbiArrayDynamicType t ->
    do
      xs <- listOf1 (scale (`div` 2) (genAbiValue t))
...
instance Arbitrary AbiValue where
  ...
  shrink = \case
    AbiArrayDynamic t v ->
      Vector.toList v
        ++ map
          (AbiArrayDynamic t . Vector.fromList)
    ...
    AbiArray _ t v ->
      Vector.toList v
        ++ map
          (\x -> AbiArray (length x) t (Vector.fromList x))
          (shrinkList shrink (Vector.toList v))
    ...
    AbiAddress a ->
      [AbiAddress 0xacab, AbiAddress 0xdeadbeef, AbiAddress 0xbabeface]
...
parseAbiValue (AbiUIntType n) = do
  W256 w <- readS_to_P reads
  return $ AbiUInt n w
parseAbiValue (AbiIntType n) = do
  W256 w <- readS_to_P reads
  return $ AbiInt n (num w)
parseAbiValue AbiAddressType = AbiAddress <$> readS_to_P reads
parseAbiValue AbiBoolType =
  ( do
      W256 w <- readS_to_P reads
      return $ AbiBool (w /= 0)
  )
    <|> ( do
            Boolz b <- readS_to_P reads
            return $ AbiBool b
        )
parseAbiValue (AbiBytesType n) =
  AbiBytes n <$> do
    ByteStringS bytes <- bytesP
    return bytes
parseAbiValue AbiBytesDynamicType =
  AbiBytesDynamic <$> do
    ByteStringS bytes <- bytesP
    return bytes
parseAbiValue AbiStringType = AbiString <$> do Char8.pack <$> readS_to_P reads
parseAbiValue (AbiArrayDynamicType typ) =
  AbiArrayDynamic typ <$> do
    a <- listP (parseAbiValue typ)
...
decodeBuf tps buf =
  if containsDynamic tps
    then NoVals
      ...
      let vs = decodeStaticArgs 0 (length tps) buf
