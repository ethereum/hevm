...
import EVM (Contract, balance, bytecode, codehash, nonce)
import EVM.Expr (bufLength)
import EVM.Solidity (SourceCache (..), SrcMap, srcMapFile, srcMapLength, srcMapOffset)
...
prettyContracts x =
  object
    ( map
        (\(a, b) -> (text (show a), prettyContract b))
        (Map.toList x)
    )
