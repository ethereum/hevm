...
import EVM (Block, Contract, EVM, balance, external, initialContract, nonce)
import EVM qualified
import EVM.ABI
import EVM.FeeSchedule qualified as FeeSchedule
import EVM.SMT
import EVM.Solvers
import EVM.Types (Addr, Expr (..), Prop (..), W256, hexText, (.&&), (./=))
import Network.Wreq
...
checkBranch solvers branchcondition pathconditions = do
  checkSat solvers (assertProps [(branchcondition .&& pathconditions)]) >>= \case
    ...
    Unsat ->
      -- if pathconditions are consistent then the condition must be false
      return $ EVM.Case False
    -- Sat means its possible for condition to hold
    Sat _ ->
      -- is its negation also possible?
