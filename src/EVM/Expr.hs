...
import Data.DoubleWord (Int256, Word128 (Word128), Word256 (Word256))
  ...
  op2
    ...
    ( \x y ->
        let msb = testBit y 255
         ...
         in if x > 256
              then if msb then maxBound else 0
              else fromIntegral $ shiftR asSigned (fromIntegral x)
    )
...
readWord idx b@(WriteWord idx' val buf)
  ...
  | otherwise = case (idx, idx') of
      (Lit i, Lit i') ->
        if i >= i' + 32 || i + 32 <= i'
          then -- the region we are trying to read is completely outside of the WriteWord
            readWord idx buf
          else -- the region we are trying to read partially overlaps the WriteWord
            readWordFromBytes idx b
...
readWordFromBytes i@(Lit idx) buf =
  let bytes = [readByte (Lit i') buf | i' <- [idx .. idx + 31]]
...
copySlice a@(Lit srcOffset) b@(Lit dstOffset) c@(Lit size) d@(ConcreteBuf src) e@(ConcreteBuf "")
  ...
  | srcOffset <= num (BS.length src),
    dstOffset < maxBytes,
    size < maxBytes =
      let hd = BS.replicate (num dstOffset) 0
  ...
  | dstOffset < maxBytes,
    size < maxBytes =
      let hd = padRight (num dstOffset) $ BS.take (num dstOffset) dst
          sl =
            if srcOffset > num (BS.length src)
...
writeWord idx val b@(WriteWord idx' val' buf)
  ...
  | otherwise =
      case (idx, idx') of
        (Lit i, Lit i') ->
          if i >= i' + 32
            then -- if we can statically determine that the write at
            ...
            -- likely to occur
              WriteWord idx' val' (writeWord idx val buf)
            else -- if we cannot statically determine freedom from
...
word256At ::
  Functor f =>
  Expr EWord ->
  (Expr EWord -> f (Expr EWord)) ->
  Expr Buf ->
  f (Expr Buf)
word256At i = lens getter setter
  where
    getter = readWord i
...
readStorage addr' slot' s@(SStore addr slot val prev) =
  if addr == addr'
    then
      if slot == slot'
        then -- if address and slot match then we return the val in this write
          Just val
...
writeStorage a@(Lit addr) k@(Lit key) v@(Lit val) store = case store of
  ...
  ConcreteStore s ->
    let ctrct = Map.findWithDefault Map.empty addr s
...
writeStorage addr key val store@(SStore addr' key' val' prev)
  | addr == addr' =
      if key == key'
        then -- if we're overwriting an existing location, then drop the write
          SStore addr key val prev
