...
import Control.Lens (ix, preview, _2)
...
import Data.Text (Text, dropEnd, intercalate, pack, splitOn, unpack)
  ...
  let ?context = DappContext {info = dapp, env = vm._env._contracts}
   in let pos =
       ...
       in case trace._traceData of
                            ...
                            Nothing ->
                              case topics of
                                [_, t2, _, _] ->
                                  ...
                                  --     bytes             data
                                  -- ) anonymous;
                                  let sig = fromIntegral $ shiftR topic 224 :: Word32
                                      usr = case maybeLitWord t2 of
                                        Just w ->
                                          pack $ show (fromIntegral w :: Addr)
                                        Nothing ->
                                          "<symbolic>"
                                   in case Map.lookup sig dapp.abiMap of
                                        Just m ->
            ...
            EntryTrace t ->
              t
            FrameTrace (CreationContext addr (Lit hash) _ _) ->
...
formatExpr = go
  where
    ...
    go = \case
      ...
      CopySlice srcOff dstOff size src dst ->
        T.unlines
          [ "(CopySlice",
            indent 2 $
              T.unlines
