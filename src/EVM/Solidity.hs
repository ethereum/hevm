...
solidity contract src = liftIO $ do
  ...
  let Contracts sol = fromRight (internalError . T.unpack $ "while trying to parse function signature `") $ readStdJSON json
...
readStdJSON json = do
  ...
  where translate :: Map Text ContractJSON -> Either BytecodeReadingError (Map Text SolcContract)
