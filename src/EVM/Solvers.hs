...
getModel inst@(SolverInstance _ _ _ _) cexvars = do
  ...
  else mkConcrete <$> execStateT (shrinkModel hints) initialModel
  where
    getRaw :: IO SMTCex
    getRaw = do
...
spawnSolver solver timeout = do
  ...
  let cmd = (proc (show solver) (fmap T.unpack $ solverArgs solver timeout)) { std_in = CreatePipe, std_out = UseHandle writeout, std_err = UseHandle writeout }
...
checkCommand inst cmd = do
  ...
  case res of
    "success" -> pure ()
    _ -> internalError $ "Internal Error: Unexpected solver output: " <> (T.unpack res)
