...
import EVM hiding (Query, Revert, bytecode, cache, push)
...
import EVM.Format (formatBinary, formatExpr, indent)
import EVM.SMT
import EVM.Solvers
...
symAbiArg name = \case
  ...
  AbiIntType n ->
    if n `mod` 8 == 0 && n <= 256
      then -- TODO: is this correct?
        let v = Var name in St [inRange n v] v
      else error "bad type"
...
loadSymVM x initStore addr callvalue' calldata' calldataProps =
  ( makeVm $
    ...
    & set
      (env . contracts . at (createAddress ethrunAddress 1))
...
verify solvers opts preState maybepost = do
  ...
  case maybepost of
    ...
    Just post -> do
      let -- Filter out any leaves that can be statically shown to be safe
...
equivalenceCheck solvers bytecodeA bytecodeB opts signature' = do
  case bytecodeA == bytecodeB of
    ...
    False -> do
      ...
      when opts.debug $
        putStrLn $
          "endstates in bytecodeA: "
            <> (show $ length branchesA)
            <> "\nendstates in bytecodeB: "
            <> (show $ length branchesB)
      ...
      when opts.debug $
        forM_
          (zip differingEndStates [(1 :: Integer) ..])
          ( \(x, i) ->
  ...
  where
    ...
    distinct (aProps, aEnd) (bProps, bEnd) =
      let differingResults = case (aEnd, bEnd) of
...
formatCex cd m@(SMTCex _ _ store blockContext txContext) =
  T.unlines $
    [ "Calldata:",
      indent 2 cd',
      ""
