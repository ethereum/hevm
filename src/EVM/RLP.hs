...
rlpdecode bs
  ...
  | otherwise = Nothing
 where
  (pre, len, isList, optimal) = itemInfo bs
  content = drop pre bs
...
encodeLen offset bs
  | BS.length bs <= 55 = prefix (BS.length bs) <> bs
