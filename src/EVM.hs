...
import Crypto.Hash (Digest, RIPEMD160, SHA256, digestFromByteString)
import Crypto.Hash qualified as Crypto
import Crypto.Number.ModArithmetic (expFast)
import Crypto.PubKey.ECC.ECDSA (PrivateKey (..), Signature (..), signDigestWith)
import Crypto.PubKey.ECC.Generate (generateQ)
import Crypto.PubKey.ECC.Types (CurveName (..), Point (..), getCurveByName)
...
import EVM.Solidity
import EVM.Types hiding (Error, IllegalOverflow)
import Options.Generic as Options
import Prelude hiding (GT, LT, exponent, log)
...
exec1 = do
  ...
  if self > 0x0 && self <= 0x9
    then do
      ...
      if vm._state._pc >= opslen vm._state._code
        then doStop
        else do
          let ?op =
                case vm._state._code of
          ...
          case ?op of
                    ...
                    let n = (num x - 0xa0)
                     in case stk of
                          (xOffset' : xSize' : xs) ->
                            if length xs < n
                              then underrun
                              else forceConcrete2 (xOffset', xSize') "LOG" $ \(xOffset, xSize) -> do
                      ...
                      case toWord64 n of
                        ...
                        Just n'' ->
                          if n'' <= ((maxBound :: Word64) - g_verylow) `div` g_copy * 32
                            then burn (g_verylow + g_copy * ceilDiv (num n) 32) $
            ...
            0x40 -> do
              ...
              stackOp1 (const g_blockhash) $ \case
                (Lit i) ->
                  if i + 256 < vm._block._number || i >= vm._block._number
                    then Lit 0
                ...
                case stk of
                  (xValue' : xOffset' : xSize' : xs) -> forceConcrete3 (xValue', xOffset', xSize') "CREATE" $
                    \(xValue, xOffset, xSize) -> do
                      accessMemoryRange fees xOffset xSize $ do
                        availableGas <- use (state . gas)
                        let newAddr = createAddress self this._nonce
                ...
                (xOffset' : xSize' : _) -> forceConcrete2 (xOffset', xSize') "RETURN" $ \(xOffset, xSize) ->
                  accessMemoryRange fees xOffset xSize $ do
                    let output = readMemory xOffset' xSize' vm
                        codesize =
                          fromMaybe (error "RETURN: cannot return dynamically sized abstract data")
                            . unlit
                            . bufLength
                            $ output
                    ...
                    if creation
                      then
                        if codesize > maxsize
                          then finishFrame (FrameErrored (MaxCodeSizeExceeded maxsize codesize))
                          else do
                            let frameReturned =
                            ...
                            case readByte (Lit 0) output of
                              ...
                              y -> do
                                ...
                                branch loc (Expr.eqByte y (LitByte 0xef)) $ \case
                                  True -> frameErrored
                                  False -> frameReturned
                      else finishFrame (FrameReturned output)
                _ -> underrun
            -- op: DELEGATECALL
            0xf4 ->
              case stk of
                    ...
                    \(xValue, xOffset, xSize, xSalt) ->
                      accessMemoryRange fees xOffset xSize $ do
                        ...
                        forceConcreteBuf (readMemory xOffset' xSize' vm) "CREATE2" $
                          \initCode -> do
                            let newAddr = create2Address self xSalt initCode
                ...
                case stk of
                  ...
                  (xTo' : _) -> forceConcrete xTo' "SELFDESTRUCT" $ \(num -> xTo) -> do
                    ...
                    let cost = if acc then 0 else g_cold_account_access
                        ...
                        c_new =
                          if not recipientExists && funds /= 0
      ...
      case preCompileAddr of
        ...
        0x3 ->
          ...
          forceConcreteBuf input "RIPEMD160" $ \input' ->
            let padding = BS.pack $ replicate 12 0
    ...
    Nothing ->
      use (cache . fetchedContracts . at addr) >>= \case
        Just c -> do
          ...
          continue c
        Nothing -> do
          assign result . Just . VMFailure $
            Query $
              PleaseFetchContract
                addr
                ( \c -> do
                    assign (cache . fetchedContracts . at addr) (Just c)
                    assign (env . contracts . at addr) (Just c)
                    assign result Nothing
                    continue c
                )
  ...
  use (env . contracts . at addr) >>= \case
    Just c ->
      case readStorage (litAddr addr) slot store of
        ...
        Nothing ->
          if c._external
            then forceConcrete slot "cannot read symbolic slots via RPC" $ \litSlot -> do
...
finalize :: EVM ()
finalize = do
  let revertContracts = use (tx . txReversion) >>= assign (env . contracts)
...
burn n continue = do
  ...
  if n <= available
    then do
      ...
      burned += n
      continue
    else vmError (OutOfGas available n)
...
cheat (inOffset, inSize) (outOffset, outSize) = do
  ...
  vm <- get
  let abi = readBytes 4 (Lit inOffset) mem
  ...
  Map.fromList
    [ action "ffi(string[])" $
        \sig outOffset outSize input -> do
          vm <- get
          if vm._allowFFI
            then case decodeBuf [AbiArrayDynamicType AbiStringType] input of
      ...
      action "sign(uint256,bytes32)" $
        \sig outOffset _ input -> case decodeStaticArgs 0 2 input of
          [sk, hash] ->
            forceConcrete2 (sk, hash) "cannot sign symbolic data" $ \(sk', hash') ->
      ...
      action "addr(uint256)" $
        \sig outOffset _ input -> case decodeStaticArgs 0 1 input of
          [sk] -> forceConcrete sk "cannot derive address for a symbolic key" $ \sk' ->
            let curve = getCurveByName SEC_p256k1
      ...
      if xTo' > 0 && xTo' <= 9
        then precompiledContract this gasGiven xTo' xContext' xValue xInOffset xInSize xOutOffset xOutSize xs
          ...
          if xTo' == cheatCode
            ...
            else callChecks this gasGiven xContext' xTo' xValue xInOffset xInSize xOutOffset xOutSize xs $
              \xGas -> do
                vm0 <- get
                    ...
                    let newContext =
                          CallContext
                            { callContextTarget = xTo',
                              ...
                              callContextAbi =
                                if xInSize >= 4
                                  then case unlit $ readBytes 4 (Lit xInOffset) vm0._state._memory of
                                    Nothing -> Nothing
...
collision c' = case c' of
  Just c ->
    c._nonce /= 0 || case c._contractcode of
        ...
        then do
          ...
          if length vm0._frames >= 1024
            then do
              ...
              if collision $ Map.lookup newAddr vm0._env._contracts
                then burn xGas $ do
                  assign (state . stack) (Lit 0 : xs)
                  assign (state . returndata) mempty
                  ...
                  case contract' of
                    ...
                    Just c -> do
                      let newContract = initialContract c
                          newContext =
                            CreationContext
                              { creationContextAddress = newAddr,
    ...
    nextFrame : remainingFrames -> do
      ...
      case nextFrame._frameContext of
        ...
        CallContext _ _ (Lit -> outOffset) (Lit -> outSize) _ _ _ reversion substate' -> do
          ...
          let substate'' = over touchedAccounts (maybe id cons (find (3 ==) touched)) substate'
        ...
        CreationContext _ _ reversion substate' -> do
          ...
          let createe = oldVm._state._contract
    ...
    Just b ->
      if 0x5b == b && OpJumpdest == snd (theCodeOps RegularVector.! (theOpIxMap Vector.! num x))
        then do
          state . stack .= xs
          state . pc .= num x
        else vmError EVM.BadJumpDestination
...
concreteModexpGasFee input =
  if lenb < num (maxBound :: Word32)
    ...
    then max 200 ((multiplicationComplexity * iterCount) `div` 3)
    else maxBound -- TODO: this is not 100% correct, return Nothing on overflow
  where
