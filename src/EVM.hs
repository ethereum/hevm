...
callChecks
  ...
  => Contract -> Gas (t :: VMType) -> Expr EAddr -> Expr EAddr -> Expr EWord -> Expr EWord -> Expr EWord -> Expr EWord -> Expr EWord -> [Expr EWord]
...
callChecks this xGas xContext xTo xValue xInOffset xInSize xOutOffset xOutSize xs continue = do
  ...
  accessMemoryRange xInOffset xInSize $
    accessMemoryRange xOutOffset xOutSize $ do
      ...
      costOfCall fees recipientExists xValue availableGas xGas xTo $ \gas' -> do
...
forceConcrete3 :: (Expr EWord, Expr EWord, Expr EWord) -> String -> ((W256, W256, W256) -> EVM t s ()) -> EVM t s ()
...
forceConcrete4 :: (Expr EWord, Expr EWord, Expr EWord, Expr EWord) -> String -> ((W256, W256, W256, W256) -> EVM t s ()) -> EVM t s ()
...
delegateCall
  :: (VMOps t, ?op :: Word8)
  => Contract -> Gas t -> Expr EAddr -> Expr EAddr -> Expr EWord -> Expr EWord -> Expr EWord -> Expr EWord -> Expr EWord
  -> [Expr EWord]
  -> (Expr EAddr -> EVM t s ())
  -> EVM t s ()
...
create2Address :: Expr EAddr -> Expr EWord -> ByteString -> EVM t s (Expr EAddr)
create2Address (LitAddr a) (Lit s) b = pure $ Concrete.create2Address a s b
create2Address (LitAddr _) _ _ = freshSymAddr
...
instance VMOps Concrete where
  ...
  costOfCall _ _ _ _ _ _ continue = continue 0
