...
callChecks
  ...
  => Contract -> Gas (t :: VMType) -> Expr EAddr -> Expr EAddr -> Expr EWord -> W256 -> W256 -> W256 -> W256 -> [Expr EWord]
...
executePrecompile
  :: (?op :: Word8, VMOps t)
  => Addr
  -> Gas t -> W256 -> W256 -> W256 -> W256 -> [Expr EWord]
  -> EVM t s ()
...
forceConcrete3 :: VMOps t => (Expr EWord, Expr EWord, Expr EWord) -> String -> ((W256, W256, W256) -> EVM t s ()) -> EVM t s ()
...
forceConcrete4 :: VMOps t => (Expr EWord, Expr EWord, Expr EWord, Expr EWord) -> String -> ((W256, W256, W256, W256) -> EVM t s ()) -> EVM t s ()
...
delegateCall
  :: (VMOps t, ?op :: Word8)
  => Contract -> Gas t -> Expr EAddr -> Expr EAddr -> Expr EWord -> W256 -> W256 -> W256 -> W256
  -> [Expr EWord]
  -> (Expr EAddr -> EVM t s ())
  -> EVM t s ()
...
create :: forall t s. (?op :: Word8, VMOps t)
  => Expr EAddr -> Contract
  -> W256 -> Gas t -> Expr EWord -> [Expr EWord] -> Expr EAddr -> Expr Buf -> EVM t s ()
...
instance VMOps Concrete where
  ...
  partial _ = error "won't happen during concrete exec"

  branch (Lit cond) continue = continue (cond > 0)
  branch _ _ = error "won't happen during concrete exec"
